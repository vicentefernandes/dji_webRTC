//https://github.com/centricular/gstwebrtc-demos/issues/172 <-

//gcc webrtc-unidirectional-h264.c $(pkg-config --cflags --libs gstreamer-webrtc-1.0 gstreamer-sdp-1.0 libsoup-2.4 json-glib-1.0) -o webrtc-unidirect
// g++ webrtc-unidirectional-h264.c $(pkg-config --cflags --libs opencv gstreamer-webrtc-1.0 gstreamer-sdp-1.0 libsoup-2.4 json-glib-1.0) -o webrtc-unidirect -fpermissive
...
/**
 * 
 sudo apt install glib-1.0 libglib2.0-dev
...
//(gpointer)(frame.data), size, 0, size, &frame, NULL /*buffer_destroy*/ );
...
create_receiver_entry (SoupWebsocketConnection * connection)
{
  ...
  GstElement *myappsrc =
      gst_bin_get_by_name (GST_BIN (receiver_entry->pipeline), "mysource");

  g_assert(myappsrc);
  ///g_assert(GST_IS_APP_SRC(myappsrc));
  ...
  g_signal_connect (myappsrc, "need-data", G_CALLBACK (cb_need_data), NULL);
  //// g_signal_connect (myappsrc, "enough-data", G_CALLBACK (stop_feed), NULL);
